...
// Хранит последнее значение(кэш) - при подписке `subscribe` всегда получит последенее значение
// из сабжекта, даже если подписка произошла после отправки значения в поток.
...
// В потоке отправляет значение только после совершения комплита, значение отправляется то,
// которое было в сабжекте непосредственно перед комплитом.
